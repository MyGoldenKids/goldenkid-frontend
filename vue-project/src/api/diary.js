...
const getDiaryByDate = async (data, success, fail) => {
	await instance.post(`diary/date`, data).then(success).catch(fail);
};

const getCalendarDiary = async (memberId, success, fail) => {
	await instance.get(`diary/calendar/${memberId}`).then(success).catch(fail);
};
